class Genome {
  ...
  void addConnection(ArrayList<connectionHistory> innovationHistory) {
    ...
    if (fullyConnected()) {
      println("connection failed"); return;
    }
    //get random nodes
  ...
  Genome crossover(Genome parent2) {
    ...
    //since all excess and disjoint genes are inherrited from the more fit parent (this Genome) the childs structure is no different from this parent 
    // with exception of dormant connections being enabled but this wont effect nodes
